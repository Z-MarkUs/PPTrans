...
CLI dry-run tests must prove the successful report is deck-text-free and the complete plan is explicitly labeled as a zero-memory-hit upper bound. Replace credential/environment loading, paid-SDK import, paid-provider construction, translation-memory opening, output-path preflight, output writing, socket creation, and other side-effect boundaries with failing doubles; a successful dry run must touch none of them, leave the source unchanged, and create no output or memory artifact. Separately assert rejection of `--output`, `--overwrite`, `--env-file`, and `--memory`, plus warning and budget failures before any provider boundary. Validation-error tests must prove private deck text and glossary terms are redacted, while allowing an explicitly selected failing path to be named. Documentation and machine output must not describe the result as a token, cost, latency, model-availability/readiness, translation-quality, or visual-fit estimate. This proves no Python socket or provider/API request; it does not turn user-selected network-mounted input paths into local storage.
...
Package-affecting changes may use disposable local builds during prerelease development. Create them in a disposable checkout or output directory, inspect and install them locally, then remove them; do not upload, attach, or describe them as release-candidate artifacts. Releasable artifacts remain subject to the release gate.

After a versioned release is separately authorized and every release prerequisite passes, build its candidate artifacts from the tagged commit. The same commands may target a disposable directory for local package validation before then:
...
A release requires explicit action-specific maintainer authorization, all applicable gates above, a clean versioned changelog entry, synchronized package/tag versions, verified artifacts, and no unresolved secret-scanning findings. Public source availability and the attribution retained in [NOTICE.md](../NOTICE.md) and [LICENSE](../LICENSE) do not themselves authorize a versioned tag, GitHub release, or package-index upload.

CI intentionally fails the package gate for every versioned tag created from this guarded tree while v2 remains prerelease work, separate release authorization is absent, the checklist is incomplete, or the live-host workflow migration is unfinished. This is a reactive artifact safeguard, not a restriction on publishing a normal source branch and not control over Git ref creation. The live repository must also use rules that restrict version-tag creation, because a tag created from an older revision can carry an older workflow. Clearing the policy requires a reviewed update to `scripts/check_release_policy.py` that records every satisfied prerequisite; creating a matching tag is not sufficient. Security CI also covers versioned tags, and its isolated weekly schedule reruns the audit across runtime, development, and review dependencies between code changes. GitHub may disable scheduled workflows after prolonged public-repository inactivity, so maintainers must monitor and re-enable that host-side schedule rather than treat it as permanent unattended monitoring.

The 2026-08-31 read-only audit of `origin/main` also found legacy `release: created` workflows for PyPI publication and application builds. A release targeting a reachable historical revision can load those historical workflow files and cannot be retroactively protected by this branch's policy script. Before any versioned tag or release operation, disable or revoke the legacy publication path and its credential, restrict version-tag and release creation in repository settings, and verify the deployed default branch contains only the reviewed workflows. These are live-host controls; a source-tree check cannot attest that they are enabled, and authorization to publish a source branch does not authorize changing them.
