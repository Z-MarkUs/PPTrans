...
`translate --dry-run` exposes this estimate for the complete inspected plan before memory lookup. Its zero-hit unit, call, source/context-character, and total serialized-request-character counts are conservative upper bounds. The largest request and ordered per-call sizes describe the zero-hit batching; cache hits can regroup remaining misses, so `translate_plan` recomputes and revalidates actual miss batches. A successful preview emits neither deck text, source path, nor glossary values; validation errors may identify a selected input path, while duplicate glossary terms are redacted. The dry-run path validates only the local provider/model rule, reads the deck and any explicit glossary, then stops: it does not load credentials or a dotenv environment, import or construct a paid-provider SDK/client, open default or custom translation memory, select or preflight an output path, write a deck, or make a provider/API request. It rejects `--output`, `--overwrite`, `--env-file`, and `--memory` rather than implying those boundaries were checked. User-selected deck and glossary paths can still reside on network-mounted filesystems. The estimate says nothing about tokens, currency cost, latency, model availability or readiness, credential or provider readiness, translation quality, or visual fit.
