...
- **Evidence:** 517 passing tests, 92.00% combined branch-aware coverage in the latest local audit, 9,346 generated property examples, cross-platform CI configuration, packaging and documentation-integrity gates, and security scanning.
...
On Windows 11 with Python 3.12.13, the committed synthetic deck completed the deterministic `inspect → identity orchestration → patch → verify` core in a **58.594 ms median** and **60.652 ms p95** over 30 measured iterations after 3 warmups. The run came from clean commit `8b98a98`, used the exact-rebuild 87,523-byte fixture with 3 slides / 41 units / 45 spans, and records its full SHA, environment, command, and timings in the [raw benchmark result](benchmarks/results/2026-08-31-stored-ooxml-windows-python312.json).
