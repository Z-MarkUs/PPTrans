...
Paid-provider SDKs are optional adapter dependencies. Load one only after that provider is explicitly selected; inspection and the identity transaction must run without importing either paid SDK. A missing selected SDK must become an actionable `ProviderConfigurationError`, `doctor --provider` must check both package and credential presence without network access, and the development suite must exercise both adapters.
...
Expanding the inspected or translated content surface is a privacy and compatibility change, especially for hidden content such as speaker notes. Keep hidden content excluded by default; adding support does not authorize default export, and changing that default requires an explicit user decision. An opt-in path must disclose and test whether selected content reaches a provider, logs, previews, or translation memory, and development must not export hidden text without explicit authorization. Do not overload `slide_part` or `DeckPlan.slide_parts` with a different content kind: introduce a neutral owning-part/content-kind model or document and test an explicit schema migration. Discover related content through validated internal relationships, reject external, duplicate, shared, or otherwise ambiguous targets, define exactly which shapes and fields are eligible, and preserve unselected or orphaned parts.
